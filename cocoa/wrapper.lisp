...
(define-objc-class "FBWindow" "NSWindow"  
  (can-become-key-window :bool ()
    T)
  
  (can-become-main-window :bool ()
    T)
...
(defun mouse-move (window event)
  (let ((rect (cocoa:frame (view window)))
        (pos (cocoa:location-in-window event)))
    (fb:mouse-moved window (cocoa:point-x pos) (- (cocoa:rect-h rect) (cocoa:point-y pos)))))

(define-objc-class "FBContentView" "NSView"
  (can-become-key-view :bool ()
    T)
  
  (accepts-first-responder :bool ()
    T)
  
  (wants-update-layer :bool ()
    T)
  
  (accepts-first-mouse/ :bool ((event :pointer))
    T)
  ...
  (mouse-down/ :void ((event :pointer))
    (fb:mouse-button-changed window :left :press (cocoa:modifier-flags event)))
  
  (mouse-up/ :void ((event :pointer))
    (fb:mouse-button-changed window :left :release (cocoa:modifier-flags event)))
  
  (right-mouse-down/ :void ((event :pointer))
    (fb:mouse-button-changed window :right :press (cocoa:modifier-flags event)))
  
  (right-mouse-up/ :void ((event :pointer))
    (fb:mouse-button-changed window :right :release (cocoa:modifier-flags event)))
  
  (other-mouse-down/ :void ((event :pointer))
    (fb:mouse-button-changed window (cocoa:button-number event) :press (cocoa:modifier-flags event)))
  
  (other-mouse-up/ :void ((event :pointer))
    (fb:mouse-button-changed window (cocoa:button-number event) :release (cocoa:modifier-flags event)))
  ...
  (scroll-wheel/ :void ((event :pointer))
    (let ((dx (cocoa:scrolling-delta-x event))
          (dy (cocoa:scrolling-delta-y event)))
      (when (cocoa:has-precise-scrolling-deltas event)
        (setf dx (/ dx 10))
        (setf dy (/ dy 10)))
      (when (or (< 0 dx) (< 0 dy))
        (fb:mouse-scrolled window dx dy))))
  ...
  (key-down/ :void ((event :pointer))
    (let ((code (cocoa:key-code event)))
      (fb:key-changed window (translate-key code) code :press (cocoa:modifier-flags event))))

  (key-up/ :void ((event :pointer))
    (let ((code (cocoa:key-code event)))
      (fb:key-changed window (translate-key code) code :release (cocoa:modifier-flags event))))
  ...
  (do-command-by-selector/ :void ((selector :sel))))
