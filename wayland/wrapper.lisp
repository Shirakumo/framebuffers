...
(defun create-shell-objects (window)
  (when (xdg-wm-base window)
    ...
    (zp! (wl:proxy-add-listener (xdg-surface window) (xdg-surface-listener (listener window)) (display window)))
    (setf (xdg-toplevel window) (wl:xdg-surface-get-toplevel (xdg-surface window)))
    (zp! (wl:proxy-add-listener (xdg-toplevel window) (xdg-toplevel-listener (listener window)) (display window)))
         ...
         (let ((cb (wl:surface-frame (surface window))))
           (unwind-protect
                (cffi:with-foreign-objects ((done :char))
                  ...
                  (zp! (wl:proxy-add-listener cb (frame-listener (listener window)) done))
                  (wl:surface-commit (surface window))
...
(defmethod fb:request-attention ((window window))
  (when (activation-manager window)
    ...
    (zp! (wl:proxy-add-listener (activation-token window) (xdg-activation-listener (listener window)) (display window)))
  ...
  (global ((registry :pointer) (id :uint32) (interface :string) (version :uint32))
    ...
    (cond ((string= interface "wl_compositor")
          ...
          ((string= interface "wl_shell")
           ...
           (when (shell-surface window)
             (zp! (wl:proxy-add-listener (shell-surface window) (shell-surface-listener (listener window)) (display window)))))
          ((string= interface "wl_seat")
           (unless (seat window)
             (setf (seat window) (np? (wl:registry-bind registry id (cffi:get-var-pointer 'wl:seat-interface) 1)))
             (when (seat window)
               (zp! (wl:proxy-add-listener (seat window) (seat-listener (listener window)) (display window))))))
          ((string= interface "xdg_wm_base")
           (setf (xdg-wm-base window) (np? (wl:registry-bind registry id wl:xdg-wm-base-interface 1)))
           (when (xdg-wm-base window)
             (zp! (wl:proxy-add-listener (xdg-wm-base window) (xdg-wm-base-listener (listener window)) (display window)))))
...
(define-listener seat-listener
  (capabilities ((seat :pointer) (caps wl:seat-capabilities))
    (cond ((and (member :keyboard caps) (null (keyboard window)))
           ...
           (when (keyboard window)
             (zp! (wl:proxy-add-listener (keyboard window) (keyboard-listener (listener window)) (display window)))))
    ...
    (cond ((and (member :pointer caps) (null (pointer window)))
           ...
           (when (pointer window)
             (zp! (wl:proxy-add-listener (pointer window) (pointer-listener (listener window)) (display window)))))
