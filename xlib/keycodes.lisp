...
(defun xkb-lookup-key (desc i)
  (let* ((names (xlib:xkb-desc-names desc))
         ...
         (name (xlib:xkb-key-name (cffi:mem-aptr (xlib:xkb-names-keys names) '(:struct xlib:xkb-key) i))))
    (or (xkb-translate-name name)
        (dolist (i (xlib:xkb-names-num-key-aliases names))
          (let ((alias (cffi:mem-aptr aliases '(:struct xlib:xkb-alias) i)))
            (when (string= name (xlib:xkb-alias-real alias))
              (let ((transl (xkb-translate-name (xlib:xkb-alias-alias alias))))
                (when transl (return transl)))))))))

(defun make-keytable (window)
  (let ((display (display window))
        (array (make-array 256))
        (min 0) (max 0))
    (cond ((xkb window)
...
(defun translate-keycode (code window)
  (when (<= 0 code 255)
    (unless *keytable*
      (setf *keytable* (make-keytable window)))
    (aref *keytable* code)))
